...
public class VetServiceImpl implements VetService {
	...
	public String deleteTimeSlot(Long vetId, String date, String time) {
		 try {
	            ...
	            System.out.println("DELETE from Vetsdb.schedule_slots_time WHERE schedule_slots_id =:slotId AND time =:\"+\"time\"");
	            scheduleRepository.deleteBySlotIdAndTime(slotId, time);
        } catch (Exception e) {
	            // Log the exception for debugging purposes
	            e.printStackTrace();
	            return "An error occurred while deleting the time slot";
	        }
		 return "time deleted succesfully";
	...
	private List<String> createTimeSlots(int startTime, int endTime) {
		...
		while (startTime1.isBefore(endTime1)) {
			timeSlots.add(startTime1.format(DateTimeFormatter.ofPattern("HH:mm")));
